...
public:
    Key getKey() const { return keyM; }
    Datum getDatum() const { return datumM; }
    Node* getNext() const { return nextM; }
...
public:
  ...
  // PROMISES: size() == 0.
    ...
    Mystring& operator[](int index);  // non-const version
    
    friend std::ostream& operator<<(std::ostream& out, const DictionaryList& dl);
